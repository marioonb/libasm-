...
while : 
    ...
    je inc                                  ; si il est = a 0, il va a la fin et renvoi rax
    mov dl, BYTE [rdi + rax]                ; met la valeur de s1 dans dl    
    cmp BYTE [rsi + rax], dl                ; conpare s2 et s1 donc byte et dl
    je inc                                  ; si elles sont egales il ca a inc
    jmp end2                                ; sinon il va a end2

inc :
    inc rax                                 ; il increment rax 
    jmp verif                               ; et recommence au debut 

end2 :
    sub dl, BYTE[rsi + rax]                 ; il sousstrait s2 a s1 dans s1 soit dl 
